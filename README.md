...
- **Smooth Ramp safety note:** Smooth Ramp does not rewrite the lamp's stored 24-hour schedule every 10 seconds. It sends only live luminance values using the lamp's manual/hand luminance command, and only when the calculated channel values change. The full schedule is stored on the ESP32 controller; the lamp receives the current output level. This should avoid repeated schedule writes to the lamp, but the lamp manufacturer's internal firmware is closed, so use Smooth Ramp at your own discretion.
